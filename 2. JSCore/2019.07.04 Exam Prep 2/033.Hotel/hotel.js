class Hotel {
    ...
    checkOut(currentBookingNumber) {
        ...
        let totalMoney = 0;
        let currentRoomType = currentRoom['roomType'];
        this.availableRooms[currentRoomType] += 1;

        this.bookings = this.bookings.filter(x => x.currentBookingNumber !== currentBookingNumber);
        //!!!!!!!!!!!!

        totalMoney = this.roomsPricing[currentRoomType] * currentRoom['nights'];
    ...
    report() {
        ...
        let middleOutput = [];
        for (const room in this.bookings) {
            let current = [];
            ...
            if (room['services']) {
                ...
                for (const currentService in room['services']) {
